...
Eigen::MatrixXcd LongLaminar::stagger_and_step_(){
    Eigen::MatrixXcd staggered_traj = Eigen::MatrixXcd::Zero(LongLaminar::get_x_0_().size()+1, LongLaminar::get_steps_()+1); // trajectory going to be generated by stagger and step
    ...
    int check_steps = static_cast<int>(check_sec / LongLaminar::get_ddt_() + 0.5); //steps of checked trajectory
    int progress_steps = static_cast<int>(progress_sec / LongLaminar::get_ddt_() + 0.5); //steps of progress
    int cycle_limit = 1E+05;
    // LongLaminar::set_steps_(check_steps); //　多分いらない?
    
    for (int i = 0; i < stagger_and_step_num; i++){
        std::cout << "\r 現在" << LongLaminar::get_t_0_()<< "時間" << std::flush;
        LongLaminar::set_t_(LongLaminar::get_t_0_() + check_sec);

        // no perturbation at first
        Eigen::MatrixXcd checked_traj = LongLaminar::get_trajectory_();
        if (LongLaminar::isLaminarTrajectory_(checked_traj)){
            ...
            LongLaminar::set_t_0_(LongLaminar::get_t_0_() + progress_sec);
            LongLaminar::set_x_0_(checked_traj.block(0, progress_steps, LongLaminar::get_x_0_().size(), 1));
        }
        else{
            std::cout << std::endl;
            bool exit = false;
            ...
            {   int local_threads = threads;
                ...
                while (true) {
                    local_counter++;
                    LongLaminar::set_x_0_(LongLaminar::perturbator_(LongLaminar::get_x_0_()));
                    Eigen::MatrixXcd checked_traj = LongLaminar::get_trajectory_();
                    if (LongLaminar::isLaminarTrajectory_(checked_traj)) {
                        #pragma omp single 
                        {
