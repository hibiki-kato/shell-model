    ...
    if (!line){
        ...
        for(int i = 0; i < param_steps; i++){
            ...
            for(int j = 0; j < param_steps; j++){
                ...
                double mean_time = local_LL.laminar_duration_mean_(trajectory);
            #pragma omp critical
        ...
        oss << "../mean_time_para/mean_laminar_time(line)_beta" << beta_begin <<"to"<< beta_end << "_nu" << nu_begin <<"to" << nu_end <<"_"<< param_steps << "times_epsilon" << epsilon << "_" << t-t_0 << "period_latter" << std::setprecision(2) << 1 / latter << "laminar0.47.npy";  // 文字列を結合する
    }
